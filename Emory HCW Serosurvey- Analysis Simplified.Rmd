---
title: "Emory HCW Serosurvey-- Analysis Simplified"
...
# merge data
  # pull in & format GADPH PUI cumulative incidence by week data
...
# demographics
  ...
  # sex
    sero$sex <- ifelse(sero$male == 1, "male",
                ifelse(sero$male == 0, "female", "not specified"))
    # freq for variable levels, freq positive, combine
  ...
  # race
    ...
    sero$race <- ifelse(sero$race_num > 1, "Multiracial",
                 ...
                 ifelse(sero$pac_isl == 1, "Native HI/Pacific Islander", "not specified"))))))
  ...
  # ethnicity
    sero$eth <- ifelse(sero$hisp == 1, "Hispanic", 
                ifelse(sero$hisp == 0, "non-Hispanic", "not specified"))
...
# combine univariate results into one datafile and table
  ...
  # format
    uni_results_demo$prop <- format(round(uni_results_demo$prop, digits=1), nsmall=1)
    uni_results_demo$perc_pos <- format(round(uni_results_demo$perc_pos, digits=1), nsmall=1)
    # n(%) for each variable category and positivity
...
* **Job location category**- collapses "job location" into categories  
...
# combine univariate results into one datafile and table
  ...
  # format
    uni_results_job_role$prop <- format(round(uni_results_job_role$prop, digits=1), nsmall=1)
    uni_results_job_role$perc_pos <- format(round(uni_results_job_role$perc_pos, digits=1), nsmall=1)
    # n(%) for each variable category and positivity
...
```{r, echo=FALSE, message=FALSE}       
  ...
  # 9 level categorization including "not specified") and hierarchy
...
# combine univariate results into one datafile and table
  ...
  # format
    uni_results_job_loc$prop <- format(round(uni_results_job_loc$prop, digits=1), nsmall=1)
    uni_results_job_loc$perc_pos <- format(round(uni_results_job_loc$perc_pos, digits=1), nsmall=1)
    # n(%) for each variable category and positivity
...
# combine univariate results into one datafile and table
  ...
  # format
    uni_results_com_cont$prop <- format(round(uni_results_com_cont$prop, digits=1), nsmall=1)
    uni_results_com_cont$perc_pos <- format(round(uni_results_com_cont$perc_pos, digits=1), nsmall=1)
    # n(%) for each variable category and positivity
...
# combine univariate results into one datafile and table
  ...
  # format
    uni_results_occ_cont$prop <- format(round(uni_results_occ_cont$prop, digits=1), nsmall=1)
    uni_results_occ_cont$perc_pos <- format(round(uni_results_occ_cont$perc_pos, digits=1), nsmall=1)
    # n(%) for each variable category and positivity
...
# combine univariate results into one datafile and table
  ...
  # format
    uni_results_ppe$prop <- format(round(uni_results_ppe$prop, digits=1), nsmall=1)
    uni_results_ppe$perc_pos <- format(round(uni_results_ppe$perc_pos, digits=1), nsmall=1)
    # n(%) for each variable category and positivity
...
# combine univariate results into one datafile and table
  ...
  # format
    uni_results_ill$prop <- format(round(uni_results_ill$prop, digits=1), nsmall=1)
    uni_results_ill$perc_pos <- format(round(uni_results_ill$perc_pos, digits=1), nsmall=1)
    # n(%) for each variable category and positivity
...
# apply function to extract output to the models
  ...
  # formatting
    ...
    # combining OR and CI into one column
      bi_results_df$OR_CI_part1 <- paste(format(round(bi_results_df$or, digits=1), nsmall=1), 
                                         format(round(bi_results_df$lci, digits=1), nsmall=1), sep=" (")
      bi_results_df$OR_CI_part2 <- paste(bi_results_df$OR_CI_part1, 
                                         format(round(bi_results_df$uci, digits=1), nsmall=1), sep=",")
...
# model 3- Aug 11
  ...
  # formatting
    ...
    # creating "variable" column
      prelim_results$variable <- ifelse((prelim_results$category %like% "Intercept"), "Intercept",
                                 ifelse((prelim_results$category %like% "week"), "Week of test",
                                 ifelse((prelim_results$category %like% "race"), "Race",
                                 ifelse((prelim_results$category %like% "eth"), "Ethnicity",
                                 ifelse((prelim_results$category %like% "log_inc"), "Log(incidence)",
                                 ifelse((prelim_results$category %like% "age_group"), "Age group",
                                 ifelse((prelim_results$category %like% "job_role"), "Job role",
                                 ifelse((prelim_results$category %like% "job_loc"), "Job location (categ)",       
                                 ifelse((prelim_results$category %like% "any_com_cont"), "Community contact",
                                 ifelse((prelim_results$category %like% "care_c"), "Contact w/ known positive patient",
                                 ifelse((prelim_results$category %like% "pt_pos"), "Contact w/ unknown positive patient",
                                 ifelse((prelim_results$category %like% "staff_pos"), "Contact w/ positive staff",
                                 ifelse((prelim_results$category %like% "ppe_use"), "PPE use (categ)", NA)))))))))))))
    # reorder columns
      prelim_results <- prelim_results[, c(6,1,2,3,4,5)]
    # cleaning category descriptions
      prelim_results$category <- str_remove(prelim_results$category, "race") 
      prelim_results$category <- str_remove(prelim_results$category, "eth") 
      prelim_results$category <- str_remove(prelim_results$category, "job_loc_cat_9") 
      prelim_results$category <- str_remove(prelim_results$category, "age_group") 
      prelim_results$category <- str_remove(prelim_results$category, "job_role") 
      prelim_results$category <- str_remove(prelim_results$category, "ppe_use_cat")
      prelim_results$category <- ifelse(prelim_results$category == "not_spec", "not specified", prelim_results$category)
      prelim_results$category <- ifelse((prelim_results$category == "any_com_cont" |
                                        prelim_results$category == "care_c" |
                                        prelim_results$category == "pt_pos"|
                                        prelim_results$category == "staff_pos"), "y/n", prelim_results$category)
    # reduce decimals
    ...
    # combining OR and CIs into one column
      prelim_results$OR_CI_part1 <- paste(format(round(prelim_results$OR, digits=1), nsmall=1), 
                                         format(round(prelim_results$LCI, digits=1), nsmall=1), sep=" (")
      prelim_results$OR_CI_part2 <- paste(prelim_results$OR_CI_part1, 
                                         format(round(prelim_results$UCI, digits=1), nsmall=1), sep=",")
      ...
      prelim_results$adj_OR_CI_unweight <- paste(prelim_results$OR_CI_part2, prelim_results$OR_CI_part3, sep="")
          prelim_results$OR_CI_part1 <- NULL
          prelim_results$OR_CI_part2 <- NULL
          prelim_results$OR_CI_part3 <- NULL
          prelim_results$OR <- NULL
          prelim_results$LCI <- NULL
          prelim_results$UCI <- NULL
          prelim_results <- prelim_results[, c(1,2,4,3)]
          prelim_results <- prelim_results[, c(1,2,3)]
...
###    AUG 31, 2020    ###
  # weights added (race, age, sex) using SURVEY package and svyglm

# model 3- with weights using svyglm       
design <- svydesign(ids=~0, weights=~weight, data=sero)    
m3w2 <- svyglm(covid ~ race + eth + log_inc +
  ...
  # formatting
    ...
    # creating "variable" column
      prelim_results_w2$variable <- ifelse((prelim_results_w2$category %like% "Intercept"), "Intercept",
                                   ifelse((prelim_results_w2$category %like% "week"), "Week of test",
                                   ifelse((prelim_results_w2$category %like% "race"), "Race",
                                   ifelse((prelim_results_w2$category %like% "eth"), "Ethnicity",
                                   ifelse((prelim_results_w2$category %like% "log_inc"), "Log(incidence)",
                                   ifelse((prelim_results_w2$category %like% "age_group"), "Age group",
                                   ifelse((prelim_results_w2$category %like% "job_role"), "Job role",
                                   ifelse((prelim_results_w2$category %like% "job_loc"), "Job location (categ)",       
                                   ifelse((prelim_results_w2$category %like% "any_com_cont"), "Community contact",
                                   ifelse((prelim_results_w2$category %like% "care_c"), "Contact w/ known positive patient",
                                   ifelse((prelim_results_w2$category %like% "pt_pos"), "Contact w/ unknown positive patient",
                                   ifelse((prelim_results_w2$category %like% "staff_pos"), "Contact w/ positive staff",
                                   ifelse((prelim_results_w2$category %like% "ppe_use"), "PPE use (categ)", NA)))))))))))))
    # reorder columns
      prelim_results_w2 <- prelim_results_w2[, c(6,1,2,3,4,5)]
    # cleaning category descriptions
      prelim_results_w2$category <- str_remove(prelim_results_w2$category, "race") 
      prelim_results_w2$category <- str_remove(prelim_results_w2$category, "eth") 
      prelim_results_w2$category <- str_remove(prelim_results_w2$category, "job_loc_cat_9") 
      prelim_results_w2$category <- str_remove(prelim_results_w2$category, "age_group") 
      prelim_results_w2$category <- str_remove(prelim_results_w2$category, "job_role") 
      prelim_results_w2$category <- str_remove(prelim_results_w2$category, "ppe_use_cat")
      prelim_results_w2$category <- ifelse(prelim_results_w2$category == "not_spec", "not specified", prelim_results_w2$category)
      prelim_results_w2$category <- ifelse((prelim_results_w2$category == "any_com_cont" |
                                           prelim_results_w2$category == "care_c" |
                                           prelim_results_w2$category == "pt_pos"|
                                           prelim_results_w2$category == "staff_pos"), "y/n", prelim_results_w2$category)
    # reduce decimals
    ...
    # combining OR and CIs into one column
      prelim_results_w2$OR_CI_part1 <- paste(format(round(prelim_results_w2$OR, digits=1), nsmall=1), 
                                            format(round(prelim_results_w2$LCI, digits=1), nsmall=1), sep=" (")
      prelim_results_w2$OR_CI_part2 <- paste(prelim_results_w2$OR_CI_part1, 
                                            format(round(prelim_results_w2$UCI, digits=1), nsmall=1), sep=", ")
      prelim_results_w2$OR_CI_part3 <- ")"
      prelim_results_w2$adj_OR_CI_svy <- paste(prelim_results_w2$OR_CI_part2, prelim_results_w2$OR_CI_part3, sep="")
          prelim_results_w2$OR_CI_part1 <- NULL
          prelim_results_w2$OR_CI_part2 <- NULL
          prelim_results_w2$OR_CI_part3 <- NULL
          prelim_results_w2$OR <- NULL
          prelim_results_w2$LCI <- NULL
          prelim_results_w2$UCI <- NULL
          prelim_results_w2 <- prelim_results_w2[, c(1,2,4,3)]
          prelim_results_w2 <- prelim_results_w2[, c(1,2,3)]
        
        
# merge two results tables      
  # format column names
    names(prelim_results_w2)[names(prelim_results_w2) == "adj_OR_CI_svy"] <- "adj_OR_weighted_svy"
  # merge  
    prelim_results_comb <- merge(prelim_results, prelim_results_w2)
    prelim_results_comb$p_val <- NULL
  # print table
    prelim_results_comb_table <- kable(prelim_results_comb, caption = "Preliminary results", format = "html") %>%
        kable_styling(full_width = F)
        add_footnote(prelim_results_comb_table, 
        c("Incidence by zip code 2 weeks prior to testing", 
          "Ref: race=white, ethnicity=non-Hisp, age group=60+, job role=other w/ no patient contact, job location category=work from home; ppe use=as recomm."), 
        notation = "symbol")               
```        
...
# Attributable fraction calculations
  # based on: https://www.publichealth.columbia.edu/research/population-health-methods/adjusted-attributable-fractions

sero_no_miss <- sero[which(sero$log_inc >= 0), ]

# setting all occupational variables to reference group-- 
  # positive staff, job role, job location, positive patient, unknown covid+ patient
    sero_pred_all_occ <- sero_no_miss
    sero_pred_all_occ$job_role  <- "Other with no patient contact"
    sero_pred_all_occ$job_loc_cat_9  <- "Work from home"
    sero_pred_all_occ$pt_pos <- 0
    sero_pred_all_occ$care_c <- 0
    sero_pred_all_occ$staff_pos <- 0
    sero_pred_all_occ$ppe_use_cat <- "as recommended"
  # apply model to new dataframe and predict number of cases
    # resource for calc CI: https://fromthebottomoftheheap.net/2018/12/10/confidence-intervals-for-glms/
    prob_10 <- data.frame(m3w2 %>% predict(sero_pred_all_occ, type = "response", se.fit=TRUE))
    save(prob_10, file = "prob_10.RData")  
    sum(prob_10$response)

    # decide on the number of bootstrap samples
      n.boot <- 1000
    # create a distribution around the predicted probabilities from prob_12 (output from predict()), where mean.prob is the predicted prob and se.prob is the SE for the predicted prob for each person
      pred.boot.all <- list() # each list element is fit from a bootstrap sample
      for(i in 1:nrow(prob_10)){
          pred.boot <- rnorm(n.boot, mean =  prob_10[i,1], sd = (prob_10[i,2])*sqrt(9610))
          pred.boot.all[[i]] <- pred.boot
          }
  # for each value (sample) in pred.boot.all, sum across all individuals to get expected cases
    exp.cases <- list()
    for(i in 1:n.boot){
      # create a vector of the ith element in every list
      sum.pred <- sum(unlist(lapply(pred.boot.all, `[[`, i)))
      #sum
      exp.cases[[i]] <- sum.pred
      }
    head(exp.cases)
    exp.cases <- unlist(exp.cases)
  # then use the median of those samples to create a new distribution of 'nonparametric' bootstrap samples 
    # have to define a function to calculate the median first
    samplemedian <- function(x, d) {
    return(median(x[d]))
    }
    dsn.expcases.med = boot(exp.cases, samplemedian, R=n.boot)
  # calculate the CI from this distribution
    dsn.expcases.ci = boot.ci(dsn.expcases.med, type="basic")
    print(dsn.expcases.ci)
...
# loading zip code shape file
  ...
  # load shape file
    shape <- st_read("/Users/juliabaker/Downloads/tl_2019_us_zcta510/tl_2019_us_zcta510.shp")
    # format
...
# format, add variables
  ...
  # drop incidence if no participants in that zip code and with n <10
    ...
    zip_latlong_shape$inc_partic_sup <- ifelse(zip_latlong_shape$n < 10, NA, zip_latlong_shape$inc_partic)
  # proportion positive
    zip_latlong_shape$prop_pos <- round(zip_latlong_shape$n_pos/zip_latlong_shape$n*100, 1)
    # suppress if n in zip < 10 or if n pos < 10
      zip_latlong_shape$prop_pos_sup <- ifelse((zip_latlong_shape$n < 10 | 
                                                zip_latlong_shape$n_pos < 10), NA,                                                                                    zip_latlong_shape$prop_pos)
    # suppress if n in zip < 100
      zip_latlong_shape$prop_pos_sup_100 <- ifelse(zip_latlong_shape$n < 100, NA, zip_latlong_shape$prop_pos)
